...
class HomescreenLawyerDetailsScreen extends StatefulWidget {
  ...
  @override
  _HomescreenLawyerDetailsScreenState createState() => _HomescreenLawyerDetailsScreenState();
}


class _HomescreenLawyerDetailsScreenState extends State<HomescreenLawyerDetailsScreen> {
...
void initState() {
  super.initState();
  _fetchReviews();
  _addView(); // Call the function to add view when the page is opened
}

Future<void> _addView() async {
  try {
    var response = await http.post(
      Uri.parse('${GlobalData().baseUrl}/api/lawyers/add-view'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ${widget.token}',
      },
      body: jsonEncode({
        'lawyer_id': widget.lawyerData['id'],  // Use lawyer ID from widget data
        'client_id': widget.userData['id'],  // Use client ID from widget data
      }),
    );
    ...
    if (response.statusCode == 200) {
      ...
      print('Failed to add view: ${response.body}');
    }
  } catch (e) {
...
Future<int> _fetchViewCount() async {
  try {
    var response = await http.get(
      Uri.parse('${GlobalData().baseUrl}/api/lawyers/${widget.lawyerData['id']}/views'),
      headers: {
        'Authorization': 'Bearer ${widget.token}',
      },
    );
    ...
    if (response.statusCode == 200) {
      ...
      return responseData.length; // Return the length of the views array
    } else {
      print('Failed to fetch view count: ${response.body}');
      return 0; // Return default value if fetching fails
    }
  } catch (e) {
...
Future<void> _fetchReviews() async {
  ...
  // Debug print to check the lawyer ID
  print('Lawyer ID: ${widget.lawyerData['lawyer_id']}');

  // Check if lawyer ID is null
          ...
          Container(
            ...
            child: Column(
              children: [
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      child: FutureBuilder<int>(
                        future: _fetchViewCount(), // Fetch view count asynchronously
                        builder: (context, snapshot) {
                          if (snapshot.connectionState == ConnectionState.waiting) {
                            ...
                            return Text('Error', style: TextStyle(color: Colors.grey));
                          } else {
                            // Show view count if data is fetched successfully
                            return Text(
                              '(${snapshot.data})', // Display view count inside brackets
                ...
                Text(
                  '${widget.lawyerData['first_name'] ?? 'N/A'} ${widget.lawyerData['last_name'] ?? 'N/A'}',
                  style: TextStyle(fontSize: 16, color: Colors.white),
                ),
                SizedBox(height: 8.0),
                                  Wrap(
                    spacing: 8.0,
                    children: _parseSpecializations(widget.lawyerData['specializations']),
                  ),
                SizedBox(height: 8.0),
                Row(
...
Widget _buildReviewWidget(int index) {
  ...
  String reviewerName = "${client['first_name'] ?? 'Anonymous'} ${client['last_name'] ?? ''}";
  double rating = (review['ratings'] as num).toDouble(); // Ensure rating is treated as double
  String reviewText = review['comment_text'] ?? "No comment provided.";
  String profilePicture = client['profile_picture'] ?? 'assets/images/default_avatar.jpg';
...
List<Widget> _parseSpecializations(dynamic specializations) {
  List<Widget> chips = [];
  if (specializations is List) {
    for (var specialization in specializations) {
      chips.add(Chip(
        label: Text(specialization),
        backgroundColor: Color(0xFF912bFF)
      ));
    }
  } else if (specializations is String && specializations.isNotEmpty) {
    chips.add(Chip(
      label: Text(specializations),
      backgroundColor: Color(0xFFc098f2)
    ));
  } else {
    chips.add(Chip(
      label: Text('No specializations listed'),
 ...
 void _navigateToWriteReviewScreen() {
  print("Testing ${widget.lawyerData} ");
  Navigator.push(
    context,
    MaterialPageRoute(
...
Future<void> _bookAppointment(String reason) async {
  var url = Uri.parse('${GlobalData().baseUrl}/api/appointments');

  String clientId = widget.userData['id'] ?? '';  // Ensure this is not null
  String lawyerId = widget.lawyerData['lawyer_id'] ?? '';  // Ensure this is not null

  if (clientId.isEmpty || lawyerId.isEmpty) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Missing client or lawyer ID. Please make sure you are logged in and try again.'),
      ),
    );
    return;
  }
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to book appointment. ${jsonDecode(response.body)['message']}')),
      );
    }
  } catch (e) {
    print('Error booking appointment: $e');
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Error booking appointment. Please check your network connection.')),
    );
  }
}
}