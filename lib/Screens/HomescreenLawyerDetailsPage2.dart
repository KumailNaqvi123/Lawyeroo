...
import 'package:project/Screens/lawyerprofile(browse)2.dart';
import 'writereview.dart';

class HomescreenLawyerDetailsScreen2 extends StatefulWidget {
  final Map<String, dynamic> lawyerData2;
  final String token;
  final Map<String, dynamic> userData;

  HomescreenLawyerDetailsScreen2({
    required this.lawyerData2,
    required this.token,
    required this.userData,
  });

  @override
  _HomescreenLawyerDetailsScreen2State createState() => _HomescreenLawyerDetailsScreen2State();
}


class _HomescreenLawyerDetailsScreen2State extends State<HomescreenLawyerDetailsScreen2> {
...
Future<void> _fetchReviews() async {
  ...
  print('Lawyer ID: ${widget.lawyerData2['id']}');

  // Check if lawyer ID is null
  if (widget.lawyerData2['id'] == null) {
    setState(() {
  ...
  try {
    var response = await http.get(
      Uri.parse('${GlobalData().baseUrl}/api/lawyers/ratings/${widget.lawyerData2['id']}'),
      headers: {
 ...
 Future<void> toggleFavorite() async {
  ...
  var requestBody = jsonEncode({
    ...
    "lawyer_id": widget.lawyerData2['id'],
  });
            ...
            Container(
              ...
              child: Column(
                children: [
                    ...
                    children: [
                      CircleAvatar(
                        ...
                        backgroundImage: NetworkImage(
                          widget.lawyerData2['profile_picture'] ?? 'default_image_url_here',
                        ),
                      ),
                      IconButton(
                        icon: Icon(
                  ...
                  Text(
                    '${widget.lawyerData2['first_name'] ?? 'N/A'} ${widget.lawyerData2['last_name'] ?? 'N/A'}',
                    style: TextStyle(fontSize: 16, color: Colors.white),
                  ),
                  SizedBox(height: 8.0),
                  Text(
                    _parseSpecializations(widget.lawyerData2['specializations']),
                    style: TextStyle(fontSize: 16, color: Colors.white),
                  ),
                  SizedBox(height: 8.0),
                  Row(
                    ...
                    children: [
                      Row(
                        ...
                        children: List.generate(5, (index) {
                          double rating = double.tryParse('${widget.lawyerData2['rating'] ?? "0"}') ?? 0.0;
                          IconData iconData;
                          Color iconColor;
                  ...
                  TextButton(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => LawyerProfilePageBrowse2(
                            lawyerData2: widget.lawyerData2,
                          ),
                        ),
                      );
                    },
                          ...
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (context) => AllReviewsScreen(
                                lawyerId: widget.lawyerData2['id'],  // Ensure this is the correct lawyer ID key
                                token: widget.token,
                              ),
                            ),
                          );
...
Widget _buildReviewWidget(int index) {
  ...
  double rating = review['ratings'] ?? 0; // Default rating
  ...
  Navigator.push(
    ...
    MaterialPageRoute(
      builder: (context) => WriteReviewScreen(
        ...
        lawyerId: widget.lawyerData2['id'],
        token: widget.token,
...
Future<void> _bookAppointment(String reason) async {
  ...
  String lawyerId = widget.lawyerData2['id'] ?? '';  // Ensure this is not null
